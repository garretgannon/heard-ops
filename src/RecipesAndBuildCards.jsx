...
export default function RecipesAndBuildCards() {
  ...
  return (
    <div className="pb-24 px-4 py-6 space-y-3">
      <h1 className="text-xl font-extrabold text-foreground mb-1">Recipes &amp; Build Cards</h1>
      <p className="text-xs text-muted-foreground mb-4">Kitchen production references and menu specs.</p>
      <button onClick={() => navigate('/recipes')} className="w-full flex items-center gap-3 bg-card border border-border rounded-xl p-4 active:scale-[0.99] transition-all">
      ...
      <button onClick={() => navigate('/build-cards')} className="w-full flex items-center gap-3 bg-card border border-border rounded-xl p-4 active:scale-[0.99] transition-all">
        <div className="h-10 w-10 rounded-xl bg-primary/15 flex items-center justify-center">
