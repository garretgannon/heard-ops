...
function CheckCard({ check, onUpdate, isManager }) {
  ...
  return (
    <div className={cn('bg-card border rounded-2xl overflow-hidden transition-all', check.status === 'failed' ? 'border-red-500/40' : check.status === 'completed' ? 'border-green-500/30' : 'border-border')}>
      <div className="p-3">
        <div className="flex items-start gap-3 mb-2">
          ...
          <div className={cn('flex items-center gap-1 px-2 py-1 rounded-2xl text-xs font-bold', statusConfig.bg)}>
        ...
        {check.temperature_value !== undefined && (
          <div className={cn('p-2 rounded-2xl text-sm font-bold mb-2', check.temp_status === 'safe' ? 'bg-green-500/15 text-green-400' : check.temp_status === 'warning' ? 'bg-amber-500/15 text-amber-400' : 'bg-red-500/15 text-red-400')}>
        ...
        {check.sanitizer_ppm !== undefined && (
          <div className="p-2 rounded-2xl bg-green-500/15 text-sm font-bold text-green-400 mb-2">
      ...
      {check.status !== 'completed' && (
        <div className="border-t border-border/50 px-3 py-2 flex gap-2 bg-muted/20">
          <button
            ...
            className="flex-1 text-xs font-bold px-3 py-1.5 rounded-2xl bg-green-500/20 text-green-400 hover:bg-green-500/30 transition-all active:scale-95"
          >
            ✓ Complete
          </button>
          {isManager && (
            <button
              onClick={handleFail}
              className="flex-1 text-xs font-bold px-3 py-1.5 rounded-2xl bg-red-500/20 text-red-400 hover:bg-red-500/30 transition-all active:scale-95"
            >
              ✗ Failed
            </button>
          )}
  ...
  return (
    <div className="pb-28">
      ...
      <div className="bg-card border-b border-border px-4 py-3 grid grid-cols-4 gap-2 lg:mt-14">
        <div className="rounded-2xl p-2 text-center" style={{ background: 'linear-gradient(180deg, rgba(255,255,255,0.09) 0%, rgba(255,255,255,0.03) 100%)', backdropFilter: 'blur(22px) saturate(160%)', WebkitBackdropFilter: 'blur(22px) saturate(160%)', boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.10), 0 8px 24px rgba(0,0,0,0.28)' }}>
      ...
      <div className="bg-card border-b border-border px-4 py-3 flex gap-2 overflow-x-auto pb-1">
        <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} className="text-xs px-3 py-1.5 rounded-2xl bg-background border border-border font-semibold text-foreground">
          <option value="all">All Status</option>
          {Object.entries(STATUS_CONFIG).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}
        </select>
        <select value={filterPhase} onChange={e => setFilterPhase(e.target.value)} className="text-xs px-3 py-1.5 rounded-2xl bg-background border border-border font-semibold text-foreground">
