...
export default function Cleaning() {
  ...
  useEffect(() => {
    const load = async () => {
      // Fetch cleaning-related prep items or side work assignments
      const tasks = await base44.entities.SideWorkAssignment.filter({
        shift_type: "any",
        task_name: { $regex: "clean" }
      }).catch(() => []);
      ...
      const mapped = tasks.map(t => ({
        ...
        standard_notes: t.standard_notes,
      }));
  ...
  return (
    <ChecklistView
      ...
      subtitle="Track daily and deep cleaning tasks"
      items={items}
