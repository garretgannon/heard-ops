...
export default function TempLogTemplates() {
  ...
  const loadTemplates = async () => {
    setLoading(true);
    try {
      const data = await base44.entities.Template.filter({ type: 'temp_log' });
      setTemplates(data);
    } catch (error) {
  ...
  const handleEdit = (id) => {
    navigate(`/temp-log-templates/${id}/edit`);
  };
  ...
  return (
    <div className="pb-24">
      ...
      <div className="p-4 space-y-3">
        {loading ? (
          ...
          <div className="text-center py-8 text-secondary-text text-sm">
            {templates.length === 0 ? 'No temperature templates yet' : 'No templates match your search'}
          </div>
        ) : (
      ...
      <button
        onClick={() => navigate('/temp-log-templates/new')}
