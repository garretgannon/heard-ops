...
export default function CleaningTemplates() {
  ...
  const loadTemplates = async () => {
    setLoading(true);
    try {
      const data = await base44.entities.Template.filter({ type: 'cleaning' });
      setTemplates(data);
    } catch (error) {
  ...
  const handleEdit = (id) => {
    navigate(`/cleaning-templates/${id}/edit`);
  };
  ...
  return (
    <div className="pb-24">
      <div className="bg-card border-b border-border p-4 sticky top-0 z-10">
        <h1 className="text-lg font-bold text-foreground mb-3">Cleaning Templates</h1>
      ...
      <div className="p-4 space-y-3">
        {loading ? (
          ...
          <div className="text-center py-8 text-secondary-text text-sm">
            {templates.length === 0 ? 'No cleaning templates yet' : 'No templates match your search'}
          </div>
        ) : (
      ...
      <button
        onClick={() => navigate('/cleaning-templates/new')}
