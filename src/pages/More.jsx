...
import {
  ShieldAlert, ShowerHead, Wrench, Users, Package, ShoppingCart,
...
const SECTIONS = [
  {
    ...
    items: [
      ...
      { label: "Cleaning Checks", icon: ShowerHead,    path: "/bathroom-checks",  color: "text-blue-400",   bg: "bg-blue-500/15" },
      { label: "Repairs",         icon: Wrench,        path: "/maintenance",      color: "text-orange-400", bg: "bg-orange-500/15" },
      { label: "Dish Machines",   icon: ClipboardList, path: "/dish-machines",    color: "text-cyan-400",   bg: "bg-cyan-500/15" },
      { label: "Cash",            icon: DollarSign,    path: "/cash",             color: "text-yellow-400", bg: "bg-yellow-500/15" },
      { label: "Incidents",       icon: AlertTriangle, path: "/incidents",        color: "text-rose-400",   bg: "bg-rose-500/15" },
    ],
  },
  {
    label: "Team",
      ...
      <div className="space-y-4">
        {visibleSections.map(section => (
          <div key={section.label}>
            ...
            <div className="bg-[#111827] border border-[#1F2937] rounded-xl divide-y divide-[#1F2937] overflow-hidden">
              {section.items.map(item => {
                ...
                return (
                  <button
                    ...
                    className="w-full flex items-center gap-3 px-3 py-2.5 active:bg-[#1C2432] transition-colors text-left"
                  >
