...
export default function LogsTopFilterBar({ search, onSearchChange, filters, onFiltersChange, onShowAdvanced }) {
  ...
  return (
    <div className="bg-card/50 border-b border-border/40 sticky top-14 sm:top-16 z-10">
      ...
      <div className="px-4 sm:px-6 py-2 flex gap-1.5 items-center border-b border-border/20 overflow-x-auto scrollbar-hide hidden" style={{ overscrollBehavior: 'contain' }}>
        <div className="relative w-32 sm:w-40">
          ...
          <input
            ...
            className="w-full h-7 pl-9 pr-8 bg-background border border-border rounded-md text-xs text-foreground focus:border-primary focus:outline-none" />
        ...
        <select
          ...
          className="h-7 px-2 text-xs font-bold bg-background border border-border rounded-md text-foreground focus:border-primary focus:outline-none flex-shrink-0">
        ...
        <button
          ...
          className={cn(
            'h-7 px-2 rounded-md border font-bold text-xs flex items-center gap-1 transition-all flex-shrink-0',
            activeFilterCount > 0 ?
