...
export default function TaskFormModal({ open, onClose, onSave }) {
  ...
  const handleSave = () => {
    if (form.name.trim()) {
      ...
      setForm({ name: "", station: "", role: "", due_time: "", photo_required: false });
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex flex-col justify-end">
      ...
      <div className="relative bg-[#0B1018] border-t border-[#1E2A3B] rounded-t-2xl z-10 flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-[#1A2235]">
          <p className="text-[15px] font-bold text-white">Add Task</p>
        ...
        <div className="overflow-y-auto px-4 py-4 space-y-3 pb-6 flex-1">
          <div>
            <label className="block text-[11px] text-gray-500 font-semibold mb-1.5">Task Name *</label>
            <input
              value={form.name}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[11px] text-gray-500 font-semibold mb-1.5">Station / Area</label>
              <input
                value={form.station}
            ...
            <div>
              <label className="block text-[11px] text-gray-500 font-semibold mb-1.5">Role</label>
              <input
                value={form.role}
